...
render = function(){
  ...
  el.innerHTML=`<header class="identity game-nav ${inner?'identity-story':''}"><div class="nav-left"><button class="nav-tool" data-action="openOverlay" data-val="map" aria-label="打开故事地图">${uiIcon('map')}<span>地图</span></button><button class="nav-tool" data-action="openOverlay" data-val="status" aria-label="查看当前状态">${uiIcon('status')}<span>状态</span></button></div><div class="hud-context"><span>${inner?'小说世界':'作者工作台'}</span><b>${inner?'你是主角':esc(state.penName)}</b></div><div class="hud-resources">${mini}</div><span class="identity-mode">${modeText}</span></header>`+el.innerHTML+(state.overlay==='map'?mapOverlay():state.overlay==='status'?statusOverlay():state.overlay==='lowAction'?lowActionOverlay():state.overlay==='hint'?hintOverlay():state.overlay==='prepAdvice'?prepAdviceOverlay():'');
};
